...
void mergeFiles(char* output_file, int n, int k)
{
    ...
    //k = caminhos
	FILE* in[k];
    //Cria k arquivos auxiliares para intercalar
	for (int i = 0; i < k; i++) {
		char fileName[2];
...
{
	// For big input file
    // Abre o arquivo a ser ordenado;
	FILE* in = openFile(input_file, "r");

	// output scratch files
    // Cria um vetor de arquivos com o tamanho do do numero de arquivos temporarios.
	FILE* out[num_ways];
	char fileName[2];
    // Cria todos os arquivos temporarios.
    ...
    // Cria vetor pra acomodar os registros mediante o tamanho da memoria ram.
	int* arr = (int*)malloc(run_size * sizeof(int));

	bool more_input = true;
	int next_output_file = 0;

	int i;//i controla a quantidade de vezes que o loop roda pra escrever nos arquivos temporarios.
	while (more_input) {
		...
		for (i = 0; i < run_size; i++) {
			if (fscanf(in, "%d ", &arr[i]) != 1) {
				more_input = false;
                // Armazena os registros no vetor até que ele encha.
				break;
			}
		}

		// sort array using merge sort
        // Ordena a porção que ele tirou do arquivo.
		mergeSort(arr, 0, i - 1);
		...
		next_output_file++;
        //Incrementa a variavel para acessar o proximo arquivo no proximo loop
	}
